// @file: cjlib_dictionary.c
/**
 * This file contains an implementation of a dictionary data structure.
 * The implementation is using the Binary search tree's representation 
 * in order to achive the best perfomance of O(log n).
*/

#include <malloc.h>
#include <memory.h>

#include "cjlib_dictionary.h"

static int get_child_hight(const struct avl_bs_tree_node restrict *src)
{

}

static int get_balance_factor(const struct avl_bs_tree_node restrict *src)
{

}

int cjlib_dict_search(struct cjlib_json_datatype_ext restrict *dst, const struct avl_bs_tree_node restrict *dict, 
                             const char restrict *key)
{

}

int cjlib_dict_insert(const cjlib_json_datatype_ext restrict *src, struct avl_bs_tree_node restrict *dict,
                      const char restrict *key)
{

}

int cjlib_dict_remove(struct avl_bs_tree_node restrict *dict, const char restrict *key)
{

}